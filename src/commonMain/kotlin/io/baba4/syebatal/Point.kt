...
fun Point.left(bfSize: Int): Point? = copy(column = column - 1).takeIf { it.isValid(bfSize) }
fun Point.top(bfSize: Int): Point? = copy(row = row - 1).takeIf { it.isValid(bfSize) }
fun Point.right(bfSize: Int): Point? = copy(column = column + 1).takeIf { it.isValid(bfSize) }
fun Point.bottom(bfSize: Int): Point? = copy(row = row + 1).takeIf { it.isValid(bfSize) }
